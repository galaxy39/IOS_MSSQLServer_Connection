...
- (void)executeQuery:(NSString*)query
{
    @synchronized (connection) {
//        if (!qualifier) {
//            [MysqlException raiseConnection:connection withFormat:@"create is unqualified. Use TRUNCATE to wipe out tables"];
//        }
        if (mysql_query(connection.connection, [query UTF8String])) {
            [MysqlException raiseConnection:connection
                                 withFormat:@"Could not perform mysql update %@ #%d:%s",query,mysql_errno(connection.connection), mysql_error(connection.connection)];
        } else {
            unsigned long long rowCount = mysql_affected_rows(connection.connection);
            affectedRows = [[NSNumber numberWithUnsignedLongLong:rowCount] retain];
